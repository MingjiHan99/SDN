...
class TopoManager():
    ...
    def add_link(self, link):
        if link.src not in self.links.keys():
            self.links[link.src.dpid] = [link]
        else:
            self.links[link.src.dpid].append(link)

        if link.dst not in self.links.keys():
            self.links[link.dst.dpid] = [link]
        else:
            self.links[link.dst.dpid].append(link)
  
    def delete_link(self, link):
        if link.src.dpid in self.links.keys():
            self.links[link.src.dpid].remove(link)
        if link.dst in self.links.keys():
            self.links[link.dst.dpid].remove(link)
